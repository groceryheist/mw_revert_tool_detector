...
from util import get_previous_time_from_index, get_api
...
class SiteInfo(object):
    ...
    def to_dict(self):
        return {'url':self.url,
                ...
                '_msgs_php_files':self._msgs_php_files,
                'local_namespaces_byname':self.local_namespaces_byname,
                'namespaces_byid':self.namespaces_byid,
                'special_aliases':self.special_aliases,
                'fallback_langs':self.fallback_langs,
                'special_prefixes':self.special_prefixes,
                'canonical_namespaces_byid':self.canonical_namespaces_byid,
                'canonical_namespaces_byname':self.canonical_namespaces_byname,
                'lang_variants':self.lang_variants,
    ...
    def __init__(self, url):
        ...
        if self.lang_variants is None:
            ...
            langSuffixes = (langcode[0].upper() + langcode[1:] for langcode in set(self.lang_variants + [self.langcode]))
            langSuffixes = [ls.replace("-","_") for ls in langSuffixes]
            

        self._msgs_php_files = [os.path.join("languages","messages","Messages{0}.php".format(langSuffix)) for langSuffix in langSuffixes]
    ...
    def _lookup_siteinfo(self, api):
        ...
        self.namespaces_byid = {v: k for k, v in self.local_namespaces_byname.items()}

        self.canonical_namespaces_byid = {int(value['id']):value['canonical']
                                          for value
                                          in result['query']['namespaces'].values()
                                          if 'canonical' in value}


        self.canonical_namespaces_byname = {value['canonical'].lower():int(value['id'])
                                          for value
                                          in result['query']['namespaces'].values()
                                          if 'canonical' in value}


        self.special_aliases = {value['realname']:value['aliases']
                                for value
    ...
    def _extractLocalizedNamespaces(self, date):
        import git
        from util import convert_php_dict, parse_localized_namespaces
        repo_path = "temp/mediawiki"
        repo = git.Repo(repo_path)

        def _extract_from_filehistory(msgs_php_file):
            commits = repo.iter_commits("master",[msgs_php_file],max_age=date,max_count=2)

            for commit in commits:
                repo.git.checkout('-f', commit)

                ns_dict = parse_localized_namespaces(os.path.join(repo_path,msgs_php_file))
